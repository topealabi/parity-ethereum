	...
	fn dispatch_pending(&self, ctx: &BasicContext) {
		...
		*pending = ::std::mem::replace(&mut *pending, Vec::new())
			...
			.filter_map(|mut pending| {
				...
				// the peer(s) we dispatch to is chosen randomly
				...
				for (peer_id, peer) in peers
					...
					.take(num_peers)
				{
					println!("looping over peers, peer_id: {}", peer_id);
					...
					if pending.query_id_history.insert(*peer_id) {
						...
						println!("making a request to: {:?}", peer_id);
						match ctx.request_from(*peer_id, pending.net_requests.clone()) {
							Ok(req_id) => {
								println!("successful request to: {:?}", peer_id);
		...
		println!("Was unable to dispatch {} requests.", pending.len());
		...
		for response in responses {
			...
			match pending.supply_response(&*self.cache, response) {
				Err(ResponseError::Validity(err)) => {
					println!("got a response on req_id: {} {:?} from: {}", req_id, response, ctx.peer());
					match err {
						...
						ValidityError::WrongTrieRoot(_, _) | ValidityError::UnresolvedHeader(_) => {
							...
							// the request is determined as `faulty` drop it
